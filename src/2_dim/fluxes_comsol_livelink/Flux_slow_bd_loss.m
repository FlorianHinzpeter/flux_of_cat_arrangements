function f = Flux_slow_bd_loss(N1,N2,x1,y1,x2,y2,rE,beta2,alpha1,alpha2)
%---------------------------------------------------------------------------------------------
...
%The loss mechanism for the intermediate is a partially absorbing boundary condition with permeability beta2.
...
model.physics.create('c1', 'CoefficientFormPDE', 'geom1');
model.physics('c1').field('dimensionless').field('S');
model.physics('c1').field('dimensionless').component({'S'});
model.physics('c1').feature.create('flux1', 'FluxBoundary', 1);
model.physics('c1').feature('flux1').selection.named('geom1_csel1_bnd');
model.physics('c1').feature.create('dir1', 'DirichletBoundary', 1);
model.physics('c1').feature('dir1').selection.named('geom1_csel3_bnd');
...
model.physics('c2').feature('flux2').selection.named('geom1_csel2_bnd');
model.physics('c2').feature.create('flux3', 'FluxBoundary', 1);
model.physics('c2').feature('flux3').selection.named('geom1_csel3_bnd');


model.mesh.create('mesh1', 'geom1');
model.mesh('mesh1').feature.create('ftri1', 'FreeTri');

model.physics('c1').name('PDE 2');
model.physics('c1').prop('ShapeProperty').set('boundaryFlux', '0');
model.physics('c1').feature('cfeq1').set('da', '0');
model.physics('c1').feature('cfeq1').set('f', '0');
model.physics('c1').feature('flux1').set('q', 'alpha1');
model.physics('c1').feature('dir1').set('r', 's0');


model.physics('c2').name('PDE');
...
model.physics('c2').feature('flux3').set('q', 'beta2');
